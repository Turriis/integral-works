...
var urlApi = "http://cencerro.com.mx/hmconsultores/api/";
  ...
  document.getElementById("lbl-name-session").innerHTML = session.name+' ('+session.privilege_name+')';
   ...
   if (session.image !== '') {
      ...
      if (session.privileges == '10') {
         document.getElementById("img-session").src="http://cencerro.com.mx/hmconsultores/uploads/clients/image_perfil/"+session.id+"/"+session.image+"";
      //Si es usuario
      } else {
         document.getElementById("img-session").src="http://cencerro.com.mx/hmconsultores/uploads/users/image_perfil/"+session.id+"/"+session.image+"";
      }
   }
}
...
function closeSession(){
  if (confirm("¿Estas seguro de cerrar sesión?") == true) {
     ...
     window.location = "http://cencerro.com.mx/hmconsultores/index.php";
  }else{
...
function checkSession2(){
  var session = JSON.parse(localStorage.getItem('sessionHM'));
  if ( session !== null ){
     window.location = 'http://cencerro.com.mx/hmconsultores/dashboard.php';
  }
}
...
function verifyPrivileges(privileges){
   ...
   if (privileges.includes(session.privileges) === false) {
      window.location.href = 'http://cencerro.com.mx/hmconsultores/page_error.php';
   }
